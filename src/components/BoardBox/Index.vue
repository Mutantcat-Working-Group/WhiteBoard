  ...
  <div class="board" :style="boardStyle" @click="handleBoardClick">
    ...
    >
      <ToolBox style="box-shadow: 0 0 2px 2px rgba(0, 0, 0, .1); margin: 0 auto;">
        ...
        >
          <template v-slot:label>
            <a href="https://www.mutantcat.org/" target="_blank" style="color: #333333;">
              <img :src="require('../../assets/images/mutantcat.png')" alt="异猫工作群" style="width: 16px; height: 16px; vertical-align: middle;">
            </a>
          </template>
        </ToolItem>
        <!-- About -->
        ...
        >
          <template v-slot:label>
            <a href="javascript: void(0);" @click="handleAbout" style="color: #333333;">
  ...
  methods: {
    ...
    handleToolClick (item, val) {
      ...
      if (!item.types.includes(_t.actionType)) {
        console.log('WhiteBoard tool disabled!')
        return false
      }
      switch (item.name) {
        case 'preview':
        ...
        case 'redo':
          ...
          break
        case 'image':
          console.log('WhiteBoard Tool image trigger!')
          break
        case 'clear':
          _t.$Modal.confirm({
            title: _t.$t('L10101'),
    ...
    handleAbout () {
      ...
      _t.$Modal.info({
        title: _t.$t('L10108'),
        width: 480,
        render: (h) => h('div', { style: { fontSize: '13px', lineHeight: '1.8', color: '#515a6e' } }, [
          h('p', { style: { margin: '0 0 12px 0' } }, _t.$t('L10109')),
          h('p', { style: { margin: '0 0 12px 0' } }, _t.$t('L10110')),
          h('p', { style: { margin: '0 0 12px 0' } }, [
            _t.$t('L10111'),
            h('a', {
              style: { color: '#2D8cF0', textDecoration: 'none' },
              attrs: { href: 'https://github.com/OXOYO/XBoard', target: '_blank', rel: 'noopener noreferrer' }
            }, 'OXOYO/XBoard'),
            _t.$t('L10112')
          ]),
          h('p', { style: { margin: 0 } }, [
